...
This vignette reproduces ideas discussed in [Beecham and Slingsby 2019](https://journals.sagepub.com/doi/10.1177/0308518X19850580), using ggplot2 to create gridmaps analysing the geography of travel-to-work between London boroughs. Gridmaps are encoded in the same way as thematic maps (using `geom_sf()`); as more information-rich data graphics, using ggplot2's layered `geoms` and `facet_grid(row~col)` to effect the gridmap arrangement; and as gridmap-arranged thematic maps (using `facet_grid(row~col)` and `geom_sf()`) to create [OD Maps](https://www.gicentre.net/featuredpapers#/woodvisualisation2010/). 
...
knitr::include_graphics("../man/figures/lb-commutes.svg")
```
...
```{r out.width = '100%', echo = FALSE}
knitr::include_graphics("../man/figures/lb-commutes-odmap.svg")
```
