...
#' pts_normalised(pts, grd, .6)
pts_normalised <- function(pts, grd, compactness) {
  # Scale points to unit square (same coord space)
  ...
  cr <- (n_rows-1) / 2
  cc <- (n_cols-1) / 2
  r_height <- (1 / (compactness + 0.001) - 1) * (n_rows - 1) + 1
  r_width <- (1 / (compactness + 0.001) - 1) * (n_cols - 1) + 1
  min_y <- min(pts$y)
  max_y <- max(pts$y)
  min_x <- min(pts$x)
  max_x <- max(pts$x)
  return(pts |>
           mutate(
             y=rescale(y, to=c(cr-r_height/2, cr+r_height/2), from=c(min_y, max_y)),
